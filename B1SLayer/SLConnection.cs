...
public class SLConnection
{
    ...
    /// </summary>
    public IDistributedCache DistributedCache { get; } = B1SLayerSettings.DistributedCache;
    ...
    private async Task<SLLoginResponse> ExecuteLoginAsync(bool expectReturn = false)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                _loginResponse.SessionId = cookies
                    .FirstOrDefault(x => x.Name.Equals("B1SESSION", StringComparison.OrdinalIgnoreCase))
                    ?.Value;
            }
        ...
        catch (FlurlHttpException ex)
        {
            try
            {
                if (ex.Call.HttpResponseMessage is null)
                {
                    throw;
                }
    ...
    {
        return new SLRequest(this, Client.Request(resource));
    }
    ...
    {
        return new SLRequest(this, Client.Request(id is string ? $"{resource}('{id}')" : $"{resource}({id})"));
    }
